...
def title_loop():
    ...
    while True:
        if os.name == "nt":
            os.system(f'title TikTok Viewbot by @Angga ^| reqs: {reqs} rps: {rps} rpm: {rpm}')
            time.sleep(0.1)

if __name__ == "__main__":
    os.system("cls" if os.name == "nt" else "clear"); os.system("title TikTok Viewbot by @Angga" if os.name == "nt" else "")
    txt = """\n
       　　/)        
     　　_(⌒)＿ 　　/)           
     　／ / ﾉ　 ヽ /ﾋE)         
     `/ｲ// /LLﾄLL|/ /        TikTok ViewBot tools    
     ｜|/ /(6　6(/ /             |==┣▇▇▇▇▇▇═─
     ｜/_/ " ヮ"ﾉ_/            Suntik Viewers
     /Y　ﾚ `ーイ /            By ./Angga_Putra
     ﾚ|　ヽ-====-＼
     ﾚヽ　/／⌒＼⌒＼               PERINGATAN:
     　 ＼ｿ 　　｜  l   Gunakan Secukupnya Saja Agar Tidak
     　　 )ヽ＿／＿／         Terkena Shadow Ban
     　 ／　　 ﾉ\n"""
    print(
        Colorate.Vertical(
